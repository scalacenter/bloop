...
main() {
    ...
    SBT_BLOOP_BENCHMARKS=(
      ...
      #"-wi 7 -i 5 -f1 -t1 $BLOOP_GIGANTIC_JMH_OPTIONS -p project=scio -p projectName=scio+test"
      #"-wi 7 -i 5 -f1 -t1 $BLOOP_GIGANTIC_JMH_OPTIONS -p project=akka -p projectName=akka-test"
      #"-wi 7 -i 5 -f1 -t1 $BLOOP_GIGANTIC_JMH_OPTIONS -p project=spark -p projectName=spark-test"
    ...
    for benchmark in "${SBT_BLOOP_BENCHMARKS[@]}"; do
      ...
      for java_home in "${JAVA_HOMES[@]}"; do
        SBT_COMMANDS+=("$JMH_CMD .*Hot(Bloop|PipelinedBloop)Benchmark.* $benchmark -jvm $java_home")
      done
    done
