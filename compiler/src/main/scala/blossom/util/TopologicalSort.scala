...
object TopologicalSort {
  ...
  def reachable(base: Project,
                projects: Map[String, Project]): Map[String, Project] = {
  ...
  def sort(projects: Map[String, Project]): Array[Array[Project]] = {
    ...
    if (projects.isEmpty) Array.empty
      ...
      val newNodes = nodes.mapValues(p =>
        p.copy(dependencies = p.dependencies.filterNot(leaves.contains)))
      Array(leaves.values.toArray) ++ sort(newNodes)
    }
  }

  def tasks(from: Project,
            projects: Map[String, Project]): Array[Array[Project]] =
