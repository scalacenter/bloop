...
handling from the build tool, mostly located in `Compiler.scala` and `CompileTask.scala`:
- We save best effort artifacts separately, and allow dependent projects to compile using 
that, even when the compilation has failed. It is 
...
expecting .betasty files
- We do not recompile if we know we are compiling the whole module to begin with (e.g. because we
