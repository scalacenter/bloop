  ...
  def writeCompileAndTestConfiguration(mojo: BloopMojo, log: Log): Unit = {
    ...
    def writeConfig(sourceDirs0: Seq[File],
      ...
      val config = {
        ...
        val jvm = Config.Jvm(Some(abs(mojo.getJavaHome())), launcher.getJvmArgs().toList)
        val project = Config.Project(name, baseDirectory, sourceDirs, dependencies, classpath, classpathOptions, classesDir, `scala`, jvm, java, test)
      ...
      Config.All.write(config, configTarget.toPath)
    }
