...
class BspMetalsClientSpec(
  ...
  private val semanticdbJar = "semanticdb-scalac_2.12.16-4.4.34.jar"
  ...
  test(
    ...
    TestUtil.withinWorkspace { workspace =>
      ...
      val defaultScalacOptions = List(
        ...
        "-P:semanticdb:synthetics:on",
        "-Xplugin-require:semanticdb",
        s"-Xplugin:path-to-plugin/semanticdb-scalac_2.12.16-4.4.30.jar.jar",
        "-Yrangepos"
      )
      val `A` = TestProject(
        workspace,
