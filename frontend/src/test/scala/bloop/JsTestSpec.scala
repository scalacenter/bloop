...
object JsTestSpec {
  ...
  def compileBeforeTesting(buildName: String, target: String): (State, Project, CompileAnalysis) = {
    ...
    val result = state.results.lastSuccessfulResultOrEmpty(project).previous.analysis().toOption
...
class JsTestSpec(
  ...
  def testsAreDetected(): Unit = {
    ...
    val client = TestUtil.defaultClient
    val classpath = testProject.fullClasspath(testState.build.getDagFor(testProject), client)
