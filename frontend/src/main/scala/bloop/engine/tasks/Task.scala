 ...
 */
class Task[+T](private val underlying: MTask[Result[T]],
               origTask: MTask[Result[T]],
  ...
  def runAsync(implicit s: Scheduler): CancelableFuture[Result[T]] = {
    underlying.runAsync
  }
  ...
  def flatMap[V](op: T => Task[V]): Task[V] = {
    val task = underlying.flatMap {
      case Success(result) => op(result).underlying
      case fail: Failure => MTask(fail)
    }
    new Task(task, task, Nil)
  }
   ...
   */
  def await(duration: Duration = Duration.Inf)(implicit s: Scheduler): Result[T] = {
