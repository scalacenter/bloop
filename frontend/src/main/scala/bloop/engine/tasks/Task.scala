 ...
 */
class Task[+T](private val underlying: MTask[T], origTask: MTask[T], dependencies: Seq[Task[_]]) {
  ...
  def runAsync(implicit s: Scheduler): CancelableFuture[Try[T]] = {
    underlying.materialize.runAsync
  }
  ...
  def flatMap[V](op: T => Task[V]): Task[V] = {
    val task: MTask[V] = underlying.flatMap(x => op(x).underlying)
    new Task(task, task, Nil)
  }
