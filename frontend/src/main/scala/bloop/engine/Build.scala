...
final case class Build private (
  ...
  def checkForChange(
    ...
    val settingsForReload = pickAndPersistSettingsForReload(currentSettings, newSettings, logger)
          ...
          case Some(LoadedProject.ConfiguredProject(project, original, settings)) =>
            findUpdateSettingsAction(Some(settings), settingsForReload) match {
              case Build.AvoidReload(_) =>
                ...
                val reattemptConfiguration = newSettings.nonEmpty && changedSettings && {
  ...
  def pickAndPersistSettingsForReload(
