...
class DebuggeeLogger(callbacks: DebugSessionCallbacks, underlying: Logger) extends Logger {
  ...
  override def asVerbose: Logger =
    new DebuggeeLogger(callbacks, underlying.asVerbose)
  override def asDiscrete: Logger =
    new DebuggeeLogger(callbacks, underlying.asDiscrete)
  override def withOriginId(originId: Option[String]): Logger =
    new DebuggeeLogger(callbacks, underlying.withOriginId(originId))
