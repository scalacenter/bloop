...
object SourceHasher {
  ...
  def findAndHashSourcesInProject(
    ...
    val waitUntilEndOrCancellation = Promise[Unit]()
    ...
    val collectAllSources = Task.create[mutable.ListBuffer[HashedSource]] { (scheduler, cb) =>
      if (isCancelled.get) {
        ...
        subscribed.success(())
        CompositeCancelable(consumerSubscription, sourceSubscription)
      }
    }

    val orderlyDiscovery = Task.fromFuture(subscribed.future).flatMap(_ => discoverFileTree)
    val aggregateTask = Task.mapBoth(orderlyDiscovery, collectAllSources) {
      case (_, sources) =>
        Task.fromFuture(waitUntilEndOrCancellation.future).map { _ =>
    ...
    aggregateTask.flatten
      .doOnFinish(_ => Task.eval { waitUntilEndOrCancellation.trySuccess(()); () })
      .doOnCancel {
        Task.eval {
          observer.onComplete()
          isCancelled.compareAndSet(false, true)
          waitUntilEndOrCancellation.success(())
        }
      }
  }
}
